...
pub fn routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/api/v1/orgs/{org_id}/deployments/by-key",
            patch(rename_by_key),
        )
        .route(
            "/api/v1/orgs/{org_id}/deployments/{id}",
            get(get_deployment).delete(delete_deployment),
        )
        .route(
            "/api/v1/orgs/{org_id}/deployments/{id}/promote",
            post(promote_draft),
        )
        .route(
            "/api/v1/orgs/{org_id}/deployments/{id}/disabled",
            patch(set_disabled),
        )
}
...
#[tracing::instrument(skip_all, fields(org_id = %org_id, process_key = %req.key))]
async fn deploy(
    State(state): State<Arc<AppState>>,
    principal: Principal,
    Path(org_id): Path<Uuid>,
    ...
    let version = process_definitions::next_version(&state.pool, org_id, &req.key).await?;
...
#[tracing::instrument(skip_all, fields(org_id = %org_id, process_key = %req.process_key))]
...
#[tracing::instrument(skip_all, fields(org_id = %org_id, id = %id, disabled = req.disabled))]
