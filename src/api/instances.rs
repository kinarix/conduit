...
pub fn routes() -> Router<Arc<AppState>> {
    Router::new()
        .route(
            "/api/v1/orgs/{org_id}/process-instances",
            get(list_instances).post(start_instance),
        )
        .route(
            "/api/v1/orgs/{org_id}/process-instances/{id}",
            get(get_instance).delete(delete_instance),
        )
        .route(
            "/api/v1/orgs/{org_id}/process-instances/{id}/pause",
            post(pause_instance),
        )
        .route(
            "/api/v1/orgs/{org_id}/process-instances/{id}/resume",
            post(resume_instance),
        )
        .route(
            "/api/v1/orgs/{org_id}/process-instances/{id}/cancel",
            post(cancel_instance),
        )
        .route(
            "/api/v1/orgs/{org_id}/process-instances/{id}/history",
            get(list_history),
        )
        .route(
            "/api/v1/orgs/{org_id}/process-instances/{id}/events",
            get(list_events),
        )
        .route(
            "/api/v1/orgs/{org_id}/process-instances/{id}/jobs",
            get(list_jobs),
        )
}
...
#[tracing::instrument(skip_all, fields(org_id = %org_id, definition_id = %req.definition_id))]
async fn start_instance(
    ...
    let instance = state
        .engine
        .start_instance(req.definition_id, org_id, &labels, &variables)
        .await?;
