...
function defName(defs: ProcessDefinition[], defId: string) {
  ...
  return d ? (d.name ?? d.process_key) : <span style={{ fontFamily: 'monospace', fontSize: 'var(--text-xs)' }}>{defId}</span>
}

export default function ProcessGroupDashboard() {
  const { org } = useOrg()
  ...
  const groupsQ = useQuery({
    queryKey: ['processGroups', org?.id],
    queryFn: () => fetchProcessGroups(org!.id),
    enabled: !!org,
  })

  const groupName = useMemo(() => {
    const g = groupsQ.data?.find(g => g.id === groupId)
    return g?.name ?? groupId ?? '…'
  }, [groupsQ.data, groupId])

  const { groupDefs, groupInst, groupTasks, processes } = useMemo(() => {
    const allDefs = deploymentsQ.data ?? []
    const allInstances = instancesQ.data ?? []
    const allTasks = tasksQ.data ?? []

    const groupDefs = allDefs.filter(d => d.process_group_id === groupId)
    const groupDefIds = new Set(groupDefs.map(d => d.id))
    const groupInst = allInstances.filter(i => groupDefIds.has(i.definition_id))
    const groupInstIds = new Set(groupInst.map(i => i.id))
    const groupTasks = allTasks.filter((t: Task) => groupInstIds.has(t.instance_id))
    const processes = groupByProcessKey(groupDefs.filter(d => d.status === 'deployed'))

    return { groupDefs, groupInst, groupTasks, processes }
  }, [deploymentsQ.data, instancesQ.data, tasksQ.data, groupId])
  ...
  const running = groupInst.filter(i => i.state === 'running')
  const errors = groupInst.filter(i => ERROR_STATES.has(i.state))
  const openTasks = groupTasks.filter(t => t.state === 'active')
  const recent = [...groupInst].sort(
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <div className={styles.subtitle}>Process group overview</div>
      </div>
      ...
      <div className={styles.panelsGrid}>
        <ErrorInstancesPanel instances={errors.slice(0, 5)} defs={groupDefs} onNavigate={navigate} onViewAll={() => navigate('/instances')} />
        <OpenTasksPanel tasks={openTasks.slice(0, 5)} defs={groupDefs} instances={groupInst} onViewAll={() => navigate('/tasks')} />
        <RecentInstancesPanel instances={recent} defs={groupDefs} onNavigate={navigate} onViewAll={() => navigate('/instances')} />
        <ProcessesPanel processes={processes} instances={groupInst} groupId={groupId!} onNavigate={navigate} />
      </div>
    </div>
  )
}
...
function OpenTasksPanel({
  ...
  function procName(instanceId: string) {
    ...
    if (!inst) return '—'
    const d = defs.find(x => x.id === inst.definition_id)
    return d ? (d.name ?? d.process_key) : '—'
  }
...
function ProcessesPanel({
  ...
  return (
    <Panel title={`Processes (${processes.length})`}>
      {processes.length === 0 ? (
        <div className={styles.empty}>No deployed processes in this group</div>
      ) : (
        <table className={styles.panelTable}>
          <thead>
            <tr><th>Name</th><th>Versions</th><th>Running</th><th>Errors</th></tr>
          </thead>
          <tbody>
            {processes.map(proc => {
              const { running, errors } = counts(proc)
              return (
                <tr key={proc.key} onClick={() => onNavigate(`/groups/${groupId}/processes/${encodeURIComponent(proc.key)}`)}>
                  <td>{proc.displayName}</td>
                  <td className={styles.muted}>{proc.versions.length}</td>
