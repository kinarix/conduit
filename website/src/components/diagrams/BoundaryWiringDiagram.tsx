export default function BoundaryWiringDiagram({ label = 'on fire', handlerLabel = 'Escalation', handlerColor = '#16a34a', handlerBg = '#f0fdf4', handlerBorder = '#16a34a' }: {
  label?: string;
  handlerLabel?: string;
  handlerColor?: string;
  handlerBg?: string;
  ...
  return (
    <div style={{ margin: '1rem 0', borderRadius: 8, background: '#f8fafc', padding: '10px 12px', border: '1px solid #e2e8f0' }}>
      ...
      <svg width={220} height={96} viewBox="0 0 220 96" style={{ display: 'block', overflow: 'visible' }}>
        ...
        <circle cx={37} cy={74} r={12} fill="#fff7ed" stroke="#ea580c" strokeWidth={1.4} strokeDasharray="3 2"/>
        <circle cx={37} cy={74} r={7} fill="none" stroke="#ea580c" strokeWidth={0.9}/>
        <path d="M37 71v3l2 1.2" stroke="#ea580c" strokeWidth={0.9} strokeLinecap="round"/>
        <circle cx={49} cy={74} r={3.5} fill="#6366f1"/>
        <text x={74} y={70} textAnchor="middle" fontSize={7} fill="#94a3b8">{label}</text>
        <line x1={53} y1={74} x2={116} y2={74} stroke="#64748b" strokeWidth={1.2}/>
        <polygon points="116,71 116,77 122,74" fill="#64748b"/>
        <rect x={122} y={67} width={56} height={14} rx={2} fill={handlerBg} stroke={handlerBorder} strokeWidth={1.2}/>
        <text x={150} y={77} textAnchor="middle" fontSize={8} fill={handlerColor}>{handlerLabel}</text>
      </svg>
    </div>
  );
}
