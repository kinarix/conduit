...
> **Inside Script Tasks**, FEEL is one of three step types — alongside `jq` (for JSON reshape) and Rhai (for general scripting). See [Script Task](/docs/elements/script-task/) for the pipeline model. Gateway conditions and DMN remain FEEL-only.
