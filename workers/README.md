...
Reference workers for the Conduit BPMN engine, shipped in the same repo under `workers/` as an independent Cargo project. Conduit orchestrates; **workers execute**. A `serviceTask` carries a `<conduit:taskTopic>` that the engine uses purely as a routing label — it knows nothing about HTTP, gRPC, Kafka, or any other transport. The crates here implement the workers that do the actual side effects.

The boundary is recorded in [ADR-008](../docs/adr/ADR-008-engine-stays-pure-bpmn.md). Phase 21 of the engine roadmap is what scopes this directory: see [`PHASE-21`](../docs/phases/PHASE-21-reference-workers.md).
...
| Crate | Topic | Status |
|---|---|---|
| `http-worker` | `http.call` | MVP (this repo's first commit) |
